...
import gov.cdc.ocio.processingstatusapi.HealthCheck.Companion.STATUS_UNSUPPORTED
import gov.cdc.ocio.processingstatusapi.HealthCheck.Companion.STATUS_UP
...
class HealthQueryService: KoinComponent {
    ...
    private fun checkAzureServiceBusHealth():HealthCheckServiceBus{
        ...
        try {
            if (isServiceBusHealthy(azureServiceBusConfiguration)) {
                serviceBusHealth.status = STATUS_UP
            }
        }catch (ex: Exception){
    ...
    private fun checkRabbitMQHealth():HealthCheckRabbitMQ {
        ...
        try {
            if (isRabbitMQHealthy(rabbitMQServiceConfiguration)) {
                rabbitMQHealth.status = STATUS_UP
            }
        }catch (ex: Exception){
    ...
    private fun checkUnsupportedMessageSystem(): HealthCheckUnsupportedMessageSystem {
        ...
        unsupportedMessageSystem.status = STATUS_UNSUPPORTED
    ...
    private fun compileHealthChecks(cosmosDBHealth: HealthCheckCosmosDb,
        ...
        return HealthCheck().apply {
            status = if (cosmosDBHealth.status == STATUS_UP && (azureServiceBusHealth?.status == STATUS_UP || rabbitMQHealth?.status == STATUS_UP || awsSQSHealth?.status == STATUS_UP)) STATUS_UP else HealthCheck.STATUS_DOWN
