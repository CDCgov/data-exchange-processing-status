...
/**
 * The `AWSSQServiceConfiguration` class configures and initializes connection AWS SQS based on
 * settings provided in an `ApplicationConfig`. This class extracts necessary AWS credentials and
 * configuration details, such as the SQS queue URL, access key, secret key, and region, using the
 * provided configuration path as a prefix.
 * @param config `ApplicationConfig` containing the configuration settings for AWS SQS.
 * @param configurationPath represents prefix used to locate environment variables specific to AWS
...
val AWSSQSPlugin =
        createApplicationPlugin(
            ...
            try {
                ...
                SchemaValidation.logger.info(
                        "Connection to the AWS SQS was successfully established"
                )
            ...
            suspend fun deleteMessage(receivedMessages: ReceiveMessageResponse) {
                receivedMessages.messages?.forEach { message ->
                    try {
                        ...
                        SchemaValidation.logger.info(
                                "Successfully deleted processed report from AWS SQS"
                        )
                    } catch (e: Exception) {
                        SchemaValidation.logger.error(
                                "Something went wrong while deleting the report from the queue ${e.message}"
                        )
                    }
                }
            }
             ...
             * @param receivedMessages the list of message(s) received from the queue to be
             * validated
             * @throws Exception thrown during validation `and` it's important to delete the message
             * as it will be persisted to dead-letter container
             * ```
            ...
            suspend fun validate(receivedMessages: ReceiveMessageResponse) {
                try {
                    receivedMessages.messages?.forEach { message ->
                        SchemaValidation.logger.info(
                                "Received message from AWS SQS: ${message.body}"
                        )
                        val awsSQSProcessor = AWSSQSProcessor()
                        message.body?.let {
                            val rawMessage = components.gson.fromJson(it, SQSMessage::class.java)

                            awsSQSProcessor.processMessage(
                                    String(Base64.getDecoder().decode(rawMessage.Message))
                            )
                        }
                    }
                    ...
                    SchemaValidation.logger.error(
                            "An Exception occurred during validation ${e.message}"
                    )
                    deleteMessage(receivedMessages)
                }
            }
            /**
             * The `consumeMessages` function continuously listens for and processes messages from
             * an AWS SQS queue. This function runs in a non-blocking coroutine, retrieving messages
             * from the queue, validating them using `AWSSQSProcessor`, and then deleting the
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    while (true) {
                        ...
                        try {
                            ...
                            SchemaValidation.logger.error(
                                    "AwsServiceException occurred while processing the request ${e.message} with requestID: ${e.sdkErrorMetadata.requestId}"
                            )
                            throw e
                        } catch (e: ClientException) {
                            SchemaValidation.logger.error(
                                    "ClientException occurred either while trying to send request to AWS or while trying to parse a response from AWS ${e.message}"
                            )
                            throw e
                        } catch (e: Exception) {
                            SchemaValidation.logger.error(
                                    "AWS service exception occurred: ${e.message}"
                            )
                            throw e
                        }
                    }
 ...
 *
 * @param application The Ktor instance, provides access to the environment monitor used for
...
/**
 * The `retryWithBackoff` retries block of code with exponential backoff, doubling the delay before
 * each retry until `maxDelay` is reached or specified number of retries is exhausted.
 *
...
suspend fun <P> retryWithBackoff(
    ...
    repeat(numOfRetries) {
        try {
            ...
            SchemaValidation.logger.error(
                    "Attempt failed with exception: ${e.message}. Retrying again in $currentDelay"
            )
            delay(currentDelay)
